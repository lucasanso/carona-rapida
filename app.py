...
lista = []
try:
    resposta = get("http://localhost:8000/passengers")
...
with st.form("formulario", clear_on_submit=False):
    ...
    if enviar:
        print(f"Cadastro realizado com sucesso! Passageiros: {passageiros}")
        st.session_state.erro_api = None
        st.session_state.sucesso_api = None

        try:
            response = post("http://localhost:8000/carona", json=dicionario)
